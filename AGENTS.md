...
- `openai-whisper` requires `setuptools<81` (older versions with `pkg_resources`). When installing dependencies, install whisper first with `--no-build-isolation`, then the main stack, then browser-use without re-resolving pins: `pip install openai-whisper==20240930 --no-build-isolation && pip install -r requirements.txt && pip install --no-deps -r requirements-browser.txt`
- `flask-socketio` is listed in `requirements.txt` (runtime dependency for collaboration).
